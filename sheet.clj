...
(defn cell-coords
  ...
  (let [[_ col row] (re-matches #"([A-Z]+)([0-9]+)" ref)
        col-index (reduce (fn [acc c] (+ (* 26 acc) (- (int c) 64))) 0 col)]
    [(dec (Integer/parseInt row)) (dec col-index)]))
...
(defn replace-refs-with-values
  [grid expr]
  (let [replaced-expr
        (str/replace (or expr nil) ref-regex #(str (resolve-ref grid %1)))]
  ...
  (let [formatted-grid (map 
                        (fn [row]
                              (map
                               ...
                               row)) grid)]
    (map (fn [row]
           (println (str (clojure.string/join row) " | "))) formatted-grid)))
